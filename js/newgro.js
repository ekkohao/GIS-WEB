$(document).ready(function(){
	...
	$('.form-newgro,.form-editgro').submit(function(e){
		...
		var mode=0;
		var gro_id=0;
		var btn=$('.form-newgro button.btn,.form-edit button.btn');
		...
		$.ajax({
		        ...
		        success: function (t) {
		        	if(t.stat==0){
		        		if(mode==1){
		        			$('.form-newgro .success-msg,.form-editgro .success-msg').html("杆塔["+old_gro_name+"]修改成功").show();
		        			...
		        			$('.form-newgro .success-msg,.form-editgro .success-msg').html("杆塔["+gro_name+"]添加成功").show();
		        		btn.attr('disabled',false);
		        		return;
		        	}
		        	else {
		        		for(var i=0;i<t.stat;i++){
		        			$('.form-newgro .error-msg,.form-editgro .error-msg').append(t.data[i]+'<br />').show();
		        		}
		        	}
		        	btn.attr('disabled',false);
		        },
		        error: function () {
		        	$('.form-newgro .error-msg,.form-editgro .error-msg').html('网络错误，请稍后再试').show();
		        	btn.attr('disabled',false);
		        }
		 });
	...
	$('.form-newgro a.new-line,.form-editgro a.new-line').click(function(){
		...
		$.ajax({
		        ...
		        success: function (t) {
		        	if(t.stat==0){
		        		...
		        		$('.form-newgro #selectLine1,.form-editgro #selectLine1').append('<option class="line-'+t.line_id+'" value="'+t.line_id+'">'+line_name+'</option>');
